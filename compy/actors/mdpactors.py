...
class MDPWorker(MDPActor):
    ...
    def __init__(self, name, service, concurrent=True, *args, **kwargs):
    ...
    def consume(self, event, *args, **kwargs):
        super(MDPWorker, self).consume(event=event, *args, **kwargs)
        if len(self._pending) > 0:
            self._executing += 1
            self.send_event(self._pending.pop(0))
        self._executing -= 1
    ...
    def process_inbound_message(self, message, *args, **kwargs):
        ...
        if message is not None:
            ...
            if command == MDPDefinition.W_REQUEST:
                ...
                if self._concurrent or self._executing == 0:
                    self._executing += 1
                    self.send_event(event)
                else:
